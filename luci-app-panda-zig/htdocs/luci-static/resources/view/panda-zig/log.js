...
/* ── injectCSS ─────────────────────────────────── */
function injectCSS() {
	if (document.getElementById('panda-zig-log-css')) return;
	var el = document.createElement('style');
	el.id = 'panda-zig-log-css';
	el.textContent = [
		'.panda-zig-log .log-pane{',
		...
		'  font-size:.75rem;',
		'}',
		'.panda-zig-log .log-bar input[type=search]:focus{',
		'  outline:2px solid rgba(137,180,250,.5);',
		'}',
		'.panda-zig-log .log-btn{',
		...
		'}',
		'.panda-zig-log .log-btn:hover{background:rgba(127,127,127,.15)}',
		'.panda-zig-log .log-btn.active{',
		'  background:rgba(64,160,43,.18);border-color:rgba(64,160,43,.35);',
		'}',
		'.panda-zig-log .log-btn.danger:hover{',
		'  background:rgba(243,139,168,.25);border-color:rgba(243,139,168,.4);',
		'}',
		'.panda-zig-log .log-btn svg{width:14px;height:14px;flex-shrink:0}',
		'.panda-zig-log .log-muted{opacity:.55;font-style:italic;font-size:.75rem}',
		'.panda-zig-log .log-stat{font-size:.75rem;opacity:.7}',
		'.panda-zig-log .log-stat strong{opacity:1;font-weight:700}',
	].join('');
...
return view.extend({
	...
	render() {
		...
		/* ── Log content ────────────────────────── */
		var logPre = E('pre', { id: 'panda-zig-log-content' });
		var logPane = E('div', { 'class': 'log-pane' }, [logPre]);

		/* ── Status display ─────────────────────── */
		var statusEl = E('span', { 'class': 'log-stat', id: 'panda-zig-log-status' }, '--');

		/* ── Search input ───────────────────────── */
		var searchInput = E('input', {
			type: 'search',
			placeholder: '搜索日志\u2026',
			id: 'panda-zig-log-search',
			input: function() {
			...
			click: function() {
				return clearLogRpc().then(function(result) {
					...
					if (result) { doReset(); }
						...
						return fs.exec_direct('/bin/sh', ['-c', ': > /var/log/panda-zig.log'])
							.then(doReset)
							.catch(function(err) {
		...
		/* ── Root ───────────────────────────────── */
		var root = E('div', { 'class': 'cbi-map panda-zig-log' }, [
			E('h2', {}, '日志'),
		...
		self.__pollHandle = poll.add(function() {
			return fs.read_direct('/var/log/panda-zig.log', 'text')
	...
	__renderLog: function() {
		var el = document.getElementById('panda-zig-log-content');
		if (!el) return;
		...
		/* Update status */
		var st = document.getElementById('panda-zig-log-status');
		if (st) {
